"use strict";
// ============================================================
// Nhost Function — triggerWorkflowRun
// Action handler for initiating a workflow execution instance
// ============================================================
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = triggerWorkflowRunHandler;
const permissions_1 = require("./lib/permissions");
async function triggerWorkflowRunHandler(req, res) {
    try {
        const { workflowId, input = {} } = req.body;
        const userId = req.headers['x-hasura-user-id'] || 'user-owner-a';
        const orgId = req.headers['x-hasura-org-id'] || 'org-a';
        const role = req.headers['x-hasura-role'] || 'owner';
        // 1. Layer 2 Permission Check
        const permCheck = (0, permissions_1.checkWorkflowPermission)({ userId, orgId, role }, 'run');
        if (!permCheck.allowed) {
            return res.status(403).json({ error: permCheck.reason });
        }
        // 2. Quota Check (Mock 100 max)
        const quotaCheck = (0, permissions_1.checkQuota)(37, 100);
        if (!quotaCheck.allowed) {
            return res.status(429).json({ error: quotaCheck.reason });
        }
        return res.status(200).json({
            success: true,
            workflowRunId: 'run_' + Math.random().toString(36).slice(2, 9),
            status: 'running',
            message: 'Workflow execution initiated successfully',
        });
    }
    catch (err) {
        return res.status(500).json({ error: err instanceof Error ? err.message : String(err) });
    }
}
//# sourceMappingURL=trigger-workflow-run.js.map